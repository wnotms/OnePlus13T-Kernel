...
make -j"$(nproc)" O="$OUT_DIR" ARCH=arm64 LLVM=1 LLVM_IAS=1 CC="ccache clang" LD=ld.lld HOSTLD=ld.lld gki_defconfig
if [[ -s "$FEATURE_CONFIG" ]]; then
 ...
 make -j"$(nproc)" O="$OUT_DIR" ARCH=arm64 LLVM=1 LLVM_IAS=1 CC="ccache clang" LD=ld.lld HOSTLD=ld.lld olddefconfig
 while IFS= read -r line; do
  [[ -z "$line" || "$line" == \#* ]] && continue
...
set -o pipefail
make -j"$(nproc)" O="$OUT_DIR" ARCH=arm64 LLVM=1 LLVM_IAS=1 CC="ccache clang" LD=ld.lld HOSTLD=ld.lld Image 2>&1 | tee "$WORK_DIR/build.log"
