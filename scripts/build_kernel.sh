...
KERNEL_UNAME=$(strings "$IMAGE" | grep -E 'Linux version .*#' | tail -n1 || true); printf '%s\n' "$KERNEL_UNAME" > "$ARTIFACT_DIR/kernel-uname.txt"
{
 ...
 echo "o2=${ENABLE_O2:-false}"; echo "lz4=${ENABLE_LZ4:-false}"; echo "zstd=${ENABLE_ZSTD:-false}"
 echo "lz4_patch_status=${LZ4_PATCH_STATUS:-not_requested}"; echo "zstd_patch_status=${ZSTD_PATCH_STATUS:-not_requested}"
